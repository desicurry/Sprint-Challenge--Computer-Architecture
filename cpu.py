import sys 

LDI = 0b10000010
HLT = 0b00000001
PRN = 0b01000111
MUL = 0b10100010
NOP = 0b00000000
POP = 0b01000110
RET = 0b00010001
CALL = 0b01010000
PUSH = 0b01000101
SP = 0b00000111
ADD = 0b10100000
SUB = 0b10100001
CMP = 0b10100111
EQ = 0b00000111
JMP = 0b01010100
JEQ = 0b01010101
JNE = 0b01010110
DIV = 0b10100011

class CPU:
    """Main CPU class."""

    def __init__(self):
        """Construct a new CPU."""
        self.ram  = [0] * 256  #RAM 265 
        self.registers= [0] * 8   # 8 REG
        self.pc = 0  # PROGRAM COUNTER 
        self.FL = 0b00000000
        self.ie = 0
        self.SP = 7
        self.hlt = False

    def ram_read(self, address):
        return self.ram[address]
    def ram_write(self, val, address):
        self.ram[address] = val


    def alu(self, op, reg_a, reg_b):

        if op == "ADD":
            self.registers[reg_a] += self.registers[reg_b]
        elif op == "MUL":
            self.registers[reg_a] *= self.registers[reg_b]
        elif op == "SUB":
            self.registers[reg_a] -= self.registers[reg_b]
        elif op == "DIV":
            if reg_b == 0:
                print("Cannot divide by zero.")
                self.hlt = True
                sys.exit(1)
            else:
                self.registers[reg_a] = self.registers[reg_a] // self.registers[reg_b]
        elif op == "CMP":
            # 00000LGE Flags
            if self.registers[reg_a] == self.registers[reg_b]:
                # set the E flag to 1
                self.FL = 0b00000001
            elif self.registers[reg_a] < self.registers[reg_b]:
                # set the L flag to 1
                self.FL = 0b00000100
            elif self.registers[reg_a] > self.registers[reg_b]:
                # set the G flag to 1
                self.FL = 0b00000010
        else:
            raise Exception("Unsupported ALU operation")



    def trace(self):
        """
        Handy function to print out the CPU state. You might want to call this
        from run() if you need help debugging.
        """

        print(f"TRACE: %02X | %02X %02X %02X |" % (
            self.pc,
            # self.fl,
            # self.ie,
            self.ram_read(self.pc),
            self.ram_read(self.pc+ 1),
            self.ram_read(self.pc+ 2)
        ), end='')

        for i in range(8):
            print(" %02X" % self.registers[i], end='')

        print()





    def load(self,filename):
        """Load a program into memory."""

        address = 0
        with open(filename) as fp:
            for line in fp:
                comment_split = line.split("#")
                num = comment_split[0].strip()
                if num == '':  # ignore blanks
                    continue
                val = int(num, 2)
                self.ram_write(val, address)
                address += 1


    def run(self):

        while not self.hlt:
            instruction = self.ram_read(self.pc)
            operand_a = self.ram_read( self.pc + 1 )
            operand_b = self.ram_read( self.pc + 2 )
            self.exe_instruction(instruction, operand_a, operand_b)

    def exe_instruction(self, instruction, operand_a, operand_b):

        if instruction == HLT:
            print("HALT APPLICATION")
            self.hlt = True
            sys.exit(1)
        elif instruction == LDI:
            # print("LDI")
            self.registers[operand_a] = operand_b
            self.pc += 3
        elif instruction == PRN:
            print(self.registers[operand_a])
            self.pc += 2
        elif instruction == MUL:
            self.alu('MUL', operand_a, operand_b)
            self.pc += 3
        elif instruction == ADD:
            self.alu('ADD', operand_a, operand_b)
            self.pc += 3
        elif instruction == PUSH:
            #add to stack
            self.registers[self.SP] -= 1
            reg_num = self.ram[self.pc + 1]
            value = self.registers[reg_num]
            address = self.registers[self.SP]
            self.ram[address] = value

            self.pc += 2
        elif instruction == POP:
            #remove form stack 

            # Increment SP.
            reg_num = self.ram[self.pc + 1]
            address = self.registers[self.SP]
            value = self.ram[address]
            self.registers[reg_num] = value
            self.registers[self.SP] += 1

            self.pc += 2

        elif instruction == CALL:
            #jump to address
            return_addr = self.pc + 2

        # push on the stack
            self.registers[self.SP] -= 1
            self.ram[self.registers[self.SP]] = return_addr

        # set the PC to the value in the given register
            reg_num = self.ram[self.pc + 1]
            dest_addr = self.registers[reg_num]
            self.pc = dest_addr
        elif instruction == RET:
                  # pop return address from top of stack
            return_addr = self.ram[self.registers[self.SP]]
            self.registers[self.SP] += 1

        # set the pc
            self.pc = return_addr

        elif instruction == SUB:
            self.alu('SUB', operand_a, operand_b)
            self.pc += 3
        elif instruction == DIV:
            self.alu('DIV', operand_a, operand_b)
            self.pc += 3
        elif instruction == CMP:
            self.alu('CMP', operand_a, operand_b)
            self.pc += 3
        elif instruction == JMP:
             # Jump to the address in the given reg
            reg_num = self.ram[self.pc + 1]
        # Set the PC to the address in the given reg
            self.pc = self.registers[reg_num]
        elif instruction == JEQ:
                    # If equal flag is true
        # jump to the address in the given reg
            reg_num = self.ram[self.pc + 1]
            if self.FL & 0b00000001 == 1:
                self.pc = self.registers[reg_num]
            else:
                self.pc += 2
        elif instruction == JNE:
                   # If Equals flag is false
        # jump to the address in the given reg
            reg_num = self.ram[self.pc + 1]
            if self.FL & 0b00000001 == 0:
                self.pc = self.registers[reg_num]
            else:
                self.pc += 2